// src/pages/Login.jsx  — updated with language selector at top
...
export default function Login() {
  ...
  const handleSendOTP = async (e) => {
    e.preventDefault();
    if (!phone.startsWith("+")) { toast.error("Use format: +91XXXXXXXXXX"); return; }
    setLoading(true);
    try {
      await sendOTP(phone);
      setStep("otp");
  ...
  return (
    <div className="min-h-screen bg-green-50 flex flex-col px-6">
      <div id="recaptcha-container" />

      {/* Language switcher — top right */}
      <div className="flex justify-end pt-4">
        <LanguageSwitcher theme="minimal" />
      </div>
      ...
      <div className="flex flex-col items-center justify-center flex-1 pb-8">
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-400 text-xs mt-1">
            Select your language above before signing in
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 w-full max-w-sm">
          {step === "phone" ? (
            <form onSubmit={handleSendOTP}>
              ...
              <p className="text-sm text-gray-400 mb-5">Enter your mobile number</p>
            ...
            <form onSubmit={handleVerifyOTP}>
              <h2 className="text-lg font-medium text-gray-800 mb-1">Enter OTP</h2>
              <p className="text-sm text-gray-400 mb-5">
                Sent to {phone}{" "}
                <button type="button" className="text-green-600 underline"
                  onClick={() => setStep("phone")}>Change</button>
              </p>
